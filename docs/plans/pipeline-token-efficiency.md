...
## 10. O-15 (2026-09-14): stage-gated + files-scoped trimming of "Orchestration flow"

O-14 above narrowed "Agent command shape" and dropped "JubarteAI Agent Identity" wholesale, but left the third-largest section of `CLAUDE.md` — `### Orchestration flow`, the 13-item numbered walkthrough of how a task moves through the app — untouched for every pipeline/child turn, including stages that never read or write application source (`specify`, `clarify`, `planning`, `plan-review`, `decompose`) and turns whose declared file scope has nothing to do with most of the section's items. O-15 closes that gap with two additional cuts inside the same `claude-md-filter.ts` module and the same `resolvePipelineSystemPromptFile` call site O-14 already wired into `orchestrator.ts` — no new module, no new call site.

Both layers key off the section's own numbered-item structure (`N. **Title**`, mirroring the bullet-parsing O-14 already does for "Agent command shape") rather than sub-headings, because `### Orchestration flow` has no sub-headings — the items are the only structure available to cut against.

- **Layer 1 — stage gate, whole tail of the section.** For the five stages that never touch application source (`specify`, `clarify`, `planning`, `plan-review`, `decompose`), the body is cut from the start of item 5's marker to the end of the section, keeping items 1–4 (the always-relevant "what agetor is and how a run starts" framing) verbatim. A build child (`isChild: true`) is never subject to Layer 1 regardless of its (always-null) `pipelineStage` — it goes straight to Layer 2.
- **Layer 2 — files-overlap, four items only.** For every other in-scope turn (`code-review`, `testing`, `building`, or a build child), the section is kept in full except for four items whose titles are matched by exact text — "Task context menu", "Tasks from issues", "Shared task-composition modules", and `` `@` file references `` — chosen because each documents a UI-only feature with no server-side code path a build/review turn would ever need to touch. An eligible item is dropped only when: it has at least one backtick-quoted, `/`-containing path hint in its own body; the turn's file list is non-empty; and every hint fails to overlap every file in that list, using the same directory-prefix overlap semantics `stage-handoff.ts`'s `inLane` already applies to child-handoff lane filtering (now exported for this reuse). An item with no extractable path hint, or a turn with no file signal at all, is always kept — the filter only ever removes text it can positively rule irrelevant, never text it merely lacks evidence for.

Both layers are gated by the existing `AGETOR_PIPELINE_CLAUDE_MD_FILTER` switch and **no new switch was added** — that switch already wraps the entire `filterClaudeMdForPipeline` call in `resolvePipelineSystemPromptFile` before either O-14's or O-15's cuts run, so setting it to `"0"` returns the raw, unfiltered `CLAUDE.md` path for every pipeline/child session exactly as it did before this change.

**The five-stage / four-group split, and why testing/building are asymmetric with code-review.** Layer 1's stage gate applies to exactly the five stages above; every other pipeline/child turn — `code-review`, `testing`, `building`, and every build child — falls through to Layer 2 instead of losing the section outright, because all four groups can plausibly need to reason about how a feature they're touching threads through the rest of the app. Within that four-group split, only `code-review` and build children currently have a real files signal to filter against: `code-review` derives it by parsing `git diff --stat` output captured earlier in the turn (`parseReviewDiffStatPaths` in `orchestrator.ts`, handling plain, arrow-rename, and brace-rename `--stat` line shapes) into `pipelineOverlapFiles`, and a build child derives it from its own subtask's `files` ownership list in `TASKS.json` (`subtaskFilesForChild`, extracted into `build-scheduler.ts` and reused from both its original DAG-scheduler call site and this new one). `testing` and `building` (the parent-task stage, not a build child) have no equivalent per-turn file signal today — no diff has been computed and no subtask-scoped file list exists at the parent level — so `pipelineOverlapFiles` returns `null` for them by construction, `files` is empty, and Layer 2's AC-7 "no signal → keep everything" rule leaves items 5–13 fully intact for those two stages. This is an intentional, documented boundary rather than an oversight: adding a `testing`/`building`-specific file signal (e.g. running `git diff --stat` against the pipeline's own base ref) was explicitly out of scope for this slice and is left as a future follow-up rather than folded in here.

**Measured saving, representative child-build turn.** Run against this repo's own `CLAUDE.md` (147,497 bytes on 2026-09-14) with `filterClaudeMdForPipeline`, isolating O-15's own marginal contribution on top of an O-14-only baseline (`{ agentKind: "claude-code", pipelineStage: null, isChild: false, files: null }`, which already narrows "Agent command shape" and drops "JubarteAI Agent Identity" but leaves "Orchestration flow" untouched — 87,452 bytes, section itself unchanged at 44,882 bytes):

- A representative build-child turn (`isChild: true`, a single-file scope such as `["src/bun/db.ts"]` that doesn't overlap any of the four filterable items' path hints, so all four are dropped) shrinks the `### Orchestration flow` section from 44,882 to 33,226 bytes — a further **26.0%** reduction on top of the O-14 baseline, worth **13.3%** off the whole O-14-filtered file (87,452 → 75,796 bytes) for that turn.
- A `specify`-stage turn (Layer 1, the stage gate) is far more aggressive since it drops items 5–13 outright: the section shrinks from 44,882 to 2,440 bytes, and the whole filtered file for that stage comes in at 45,010 bytes versus the 147,497-byte unfiltered original.

These are one-off manual probes against the shipped `filterClaudeMdForPipeline`/`resolvePipelineSystemPromptFile` functions (not a fabricated estimate); the automated suite backing the behavior itself is `src/bun/claude-md-filter.test.ts` (25 tests covering both layers against synthetic fixtures) and the O-15 block in `src/bun/orchestrator-pipeline.test.ts` (`subtaskFilesForChild`, `parseReviewDiffStatPaths`, `pipelineOverlapFiles`, the 4-arg `resolvePipelineSystemPromptFile` across all four stage/child cases, and one end-to-end build-child spawn test asserting the written `.agetor/CLAUDE.filtered.md` drops the non-overlapping filterable items while keeping the overlapping one) — all passing as of this writing.

**Kill switch**: `AGETOR_PIPELINE_CLAUDE_MD_FILTER=0` disables O-14 and O-15 together, restoring the unfiltered `CLAUDE.md` byte for byte for every pipeline/child session, unchanged from O-14's own kill-switch behavior.
