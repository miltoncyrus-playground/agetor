...
## 11. W3 follow-up: nothing auto-hides + pipeline-status affordances (implemented)

User decision after Phase 1 shipped: un-hide Review and Done too, using "Option B geometry + Option C's Done rule", plus attention strip, age badges, stage breakdown, and waiting-first ordering (empty-column hint text was dropped as moot — B's stubs have no body to hint in).

- **All six columns pinned; empty columns render as slim stubs.** `ALWAYS_VISIBLE_DISPLAY_COLUMNS` now holds all six; an empty column shrinks to a ~36px vertical strip (`Column.tsx`) that stays a live drop target with an isOver highlight. Deliberate deviation from the pitched "expand on drag-near": growing a column mid-drag shifts every sibling droppable's cached rect (dnd-kit measures at drag start) and misaligns the rest of the drag, so stubs expand only by gaining a card.
- **Done shows a 7-day recency window** (`partitionRecentDone`, `DONE_RECENCY_DAYS`): older finishes collapse behind a per-column "+N older" toggle; the count badge always shows the true total.
- **Attention strip** (`AttentionStrip.tsx`, `attentionSummary`): "N running · N blocked · N in review · N waiting on you" above the lanes, computed over the filter-respecting `visibleTasks`. The three status chips toggle the existing `statusFilter` (single-status focus, click again to clear); waiting is informational since pending interactions are orthogonal to columns.
- **Age badges** (`formatAge`, `AGE_BADGE_MIN_MS`): time-in-column on Review/Blocked cards after one hour of dwell, off `task.updatedAt` (bumped by the column transition). Refresh via a shared 30s ticker (`lib/minute-tick.ts`, `useSyncExternalStore`) so no `now` prop invalidates the memo'd Column/TaskCard layers.
- **Stage breakdown** (`stageBreakdown`): the lane header's merged In Progress chip is annotated with the real stages it hides ("2 building · 1 testing"), in `COLUMNS` order.
