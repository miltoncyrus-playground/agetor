...
// docs/plans/add-claude-opus-5-5.md §8 A1.
...
// reasoning_options at all, same treatment as every gemini model. The first
// twelve were live-probed on fx 0.0.10; spacexai/grok-4.7 (2026-09-21) rests
// on the Gateway catalog entry alone (docs/plans/add-grok-4-7.md §8 A1).
...
test("exactly the sixteen catalog-gated Gateway ids are catalogOnly in AGENT_OPTIONS.fx, and no other kind's models use catalogOnly", () => {
  const expectedCatalogOnly = new Set([
    ...
    "spacexai/grok-4.7",
    // 2026-09-22 (docs/plans/add-claude-opus-5-5.md) — signed-in presence
    // unverified, same reason as grok-4.7.
