...
test("migration 043 creates run_usage and run_usage_seen on a fresh DB", () => {
  const names = db
  ...
  const applied = db.query<{ id: string }, []>(`SELECT id FROM _migrations WHERE id = '043_run_usage'`).get();
  expect(applied?.id).toBe("043_run_usage");
});
