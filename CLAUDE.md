...
- **`claude-code`** → driven through `src/bun/claude-tmux.ts`. One tmux session per task hosts the interactive `claude` REPL. Launch argv looks like `claude [--model claude-opus-4-7] [--dangerously-skip-permissions | --permission-mode <id>]` (no `--print` — `claude -p` would draw from a separate Agent SDK credit starting 2026-06-15, so we use the regular subscription quota via interactive mode). The prompt is delivered as keystrokes via `tmux load-buffer + paste-buffer + send-keys Enter`. Structured output comes from tailing `~/.claude/projects/<encoded-cwd>/<sessionId>.jsonl`. Effort maps to `CLAUDE_CODE_EFFORT_LEVEL` env. Tmux is a hard prereq — `checkAgent` reports unavailable if `tmux -V` fails. Backgrounded shells (`Bash(run_in_background:true)`) are tracked as `bg_session` subagents by `claude-subagents.ts` (detected off the main JSONL, tailed with per-batch byte caps, staled out after a timeout ceiling) and surface as read-only Terminal-glyph tabs in the RunPanel strip; disable with `AGETOR_TRACK_BG_SHELLS=0`. See docs/plans/fix-bg-shell-detection.md.
...
Defaults preserve hands-off behavior: `null` `task.mode` becomes `auto` (`--dangerously-skip-permissions` for claude-code, `--sandbox workspace-write` for codex, `--yolo` for gemini). Unknown mode/model ids are passed through verbatim so unreleased options "just work" without code changes.
...
Override per-agent at runtime with env vars: `AGETOR_CLAUDE_BIN`, `AGETOR_CLAUDE_ARGS`, `AGETOR_CODEX_BIN`, `AGETOR_CODEX_ARGS`, `AGETOR_GEMINI_BIN`, `AGETOR_GEMINI_ARGS`, `AGETOR_TMUX_BIN`. Tests use `/bin/echo` via these overrides for codex/gemini and the `agent-status` probe, and `AGETOR_CLAUDE_DRIVER=fake` / `AGETOR_CODEX_DRIVER=fake` / `AGETOR_GEMINI_DRIVER=fake` to bypass tmux + the real CLI entirely.
